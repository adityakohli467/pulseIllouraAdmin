...
function ProductionSummaryContent() {
  ...
  const { data, isLoading, isFetching, refetch } = useQuery<ProductionSummary>({
    queryKey: ["ellora-production-summary", date],
    queryFn: async () => {
      const params = date ? { date } : {}
      const res = await api.get("/admin/ellora/orders/production-summary", { params })
      return res.data
    },
  })
    ...
    <div className="bg-gray-50 min-h-screen p-6 print:bg-white print:p-0" style={{ fontFamily: "Albert Sans" }}>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
          <div>
            <Link href="/admin/ellora/orders" className="print:hidden inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-2">
