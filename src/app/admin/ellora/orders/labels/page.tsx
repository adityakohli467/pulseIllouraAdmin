...
function LabelsContent() {
  ...
  const { data, isLoading } = useQuery<LabelsResponse>({
    queryKey: ["ellora-order-labels", date],
    queryFn: async () => {
      const params = date ? { date } : {}
      const res = await api.get("/admin/ellora/orders/labels", { params })
      return res.data
    },
  })
    ...
    <div className="bg-gray-100 min-h-screen p-6 print:bg-white print:p-0" style={{ fontFamily: "Albert Sans" }}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-5 print:hidden">
          <div>
            <Link href="/admin/ellora/orders" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-1">
