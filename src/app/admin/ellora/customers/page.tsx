...
export default function ElloraCustomersPage() {
  const [searchQuery, setSearchQuery] = useState("")

  const { data, isLoading } = useQuery({
    queryKey: ["ellora-customers"],
    queryFn: async () => {
      const response = await api.get("/admin/ellora/customers")
      return response.data
    },
  })

  const customers: ElloraCustomer[] = data?.customers || []
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-8" style={{ fontFamily: "Albert Sans" }}>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-4xl font-bold text-gray-900">Illoura Customers</h1>
          <p className="text-gray-500 mt-1">Staff who have placed Illoura orders.</p>
        </div>
      </div>
