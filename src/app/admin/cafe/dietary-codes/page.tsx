...
export default function CafeDietaryCodesPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: any) => api.post("/admin/cafe/dietary-codes", payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cafe-dietary-codes"] })
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, ...payload }: any) => api.put(`/admin/cafe/dietary-codes/${id}`, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cafe-dietary-codes"] })
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/admin/cafe/dietary-codes/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cafe-dietary-codes"] })
  ...
  const handleSave = () => {
    ...
    if (selected) {
      updateMutation.mutate({ id: selected.cafe_dietary_code_id, ...payload })
    } else {
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-50">
            {isLoading ? (
              ...
              <tr key={code.cafe_dietary_code_id} className="hover:bg-gray-50/50 transition-colors">
        ...
        <DialogContent className="max-w-md bg-white p-8 rounded-2xl">
          <div className="text-center">
            ...
            <div className="flex gap-4">
              ...
              <Button onClick={() => selected && deleteMutation.mutate(selected.cafe_dietary_code_id)} disabled={deleteMutation.isPending} className="flex-1 h-12 rounded-xl bg-red-600 hover:bg-red-700 text-white font-semibold">Delete</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
