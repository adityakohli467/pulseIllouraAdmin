...
export default function CafeCategoriesPage() {
  ...
  const [selected, setSelected] = useState<CafeCategory | null>(null)
  ...
  const categories: CafeCategory[] = data?.categories || []
  ...
  const createMutation = useMutation({
    mutationFn: (payload: any) => api.post("/admin/cafe/categories", payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cafe-categories"] })
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, ...payload }: any) => api.put(`/admin/cafe/categories/${id}`, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cafe-categories"] })
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/admin/cafe/categories/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cafe-categories"] })
  ...
  const handleSave = () => {
    ...
    if (selected) {
      updateMutation.mutate({ id: selected.cafe_category_id, ...payload })
    } else {
      createMutation.mutate(payload)
    }
  }

  const handleEdit = (category: CafeCategory) => {
  ...
  const handleDelete = (category: CafeCategory) => {
    setSelected(category)
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-8" style={{ fontFamily: "Albert Sans" }}>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-4xl font-bold text-gray-900">Cafe Categories</h1>
          <p className="text-gray-500 mt-1">Manage menu categories for the Cafe staff ordering portal.</p>
        </div>
        <Button
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-50">
            {isLoading ? (
              ...
              <tr key={category.cafe_category_id} className="hover:bg-gray-50/50 transition-colors">
        ...
        <DialogContent className="max-w-md bg-white p-8 rounded-2xl">
          <div className="text-center">
            ...
            <div className="flex gap-4">
              ...
              <Button onClick={() => selected && deleteMutation.mutate(selected.cafe_category_id)} disabled={deleteMutation.isPending} className="flex-1 h-12 rounded-xl bg-red-600 hover:bg-red-700 text-white font-semibold">Delete</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
