...
export default function CafeOrdersPage() {
  const [detailsOrder, setDetailsOrder] = useState<CafeManagerOrder | null>(null)
  ...
  const { data, isLoading, refetch, isFetching } = useQuery<ManagerOrdersResponse>({
    queryKey: ["cafe-manager-orders", dateMode, customDate],
    queryFn: async () => {
      const params = dateParam ? { date: dateParam } : {}
      const response = await api.get("/admin/cafe/orders", { params })
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            onClick={() => {
              ...
              window.open(`/admin/cafe/orders/production-summary`, "_blank")
            }}
            className="flex items-center gap-2 h-11 px-5 rounded-lg border-gray-200 text-gray-700"
          >
          ...
          <Button
            onClick={() => {
              ...
              window.open(`/admin/cafe/orders/labels`, "_blank")
            }}
